...
- **Measured basis = five bench runs: `run 3/4/5/6/8`** in `src/solver/data/new runs/`, plus **sixteen further real bench runs** in `src/solver/data/newest runs/` (a 3×3 flow×concentration grid, dated 2026-06-26 to 2026-07-31; 18 files logged, 2 excluded, 7 of the 9 cells replicated) — 21 real runs total, both processed by the `breakthrough_fit/` package. These are the **only** real data. Per-run provenance (source file, SHA-256, self-stated conditions, derived metrics, flags) is in `src/docs/experiments_passport.json`, generated by `src/solver/build_experiments_passport.py`.
...
- **Four files state their flow rate twice and disagree with themselves** (found 2026-08-09): the prose block at cell (0,0) says one thing, the numeric label/value cell `v` says another. `parse.py` reads `v`, so these four were fitted at the numeric value. Comparing t_b against the same-concentration runs shows the **prose** value is the physical one in all four cases. Affected: `2026-07-22-conc10-flow0.05` (prose 50 / cell 100), `2026-07-22-conc10-flow0.10` (150 / 100), `2026-07-31-conc10-flow0.10` (100 / 50), `2026-07-31-conc5-flow0.15` (150 / 100). Consequence: `q_dyn`, `L_MTZ` and `ψ` for those four are **provisional** (all scale with flow); `t_b`/`t_E`/`t₅₀` are unaffected. Do not silently pick a side — owner: lab. Full record in `src/docs/experiments_passport.json`.
- The `*ml_*g.csv` and `May-*.csv` files are **synthetic/placeholder** — never present them as measured results. They fed only a synthetic-validation appendix (now archived). **`breakthrough_fit/cross_run_figs.py` (fig9–fig12 in `src/img/generated/`) is built on them**: its "clean runs" selector is `^(\d+)ml_(\d+)g$` and fig11/fig12 plot the `May-*` records, so **none of those four figures may go in the report**. Use `src/solver/report_figs_measured.py` (R1–R4 in `src/img/generated/report/`), which is on the real 21-run basis.
- **Two `breakthrough_out` trees exist and only one is clean** (found 2026-08-09). `src/solver/breakthrough_out/` holds all 40 run directories including the five old runs; the repo-root `breakthrough_out/` holds only the 16 grid runs and is what `new_runs_pipeline.py` and `build_experiments_passport.py` write to. **Nine of the root tree's 16 `results_*.csv` carry unresolved git merge-conflict markers** (`<<<<<<< HEAD` / `=======` / `>>>>>>> ed48a6a`) and parse as 51 rows instead of 24. Both conflict sides agree to ~1e-14 relative and side A is byte-identical to the `src/solver/` copy. **Read statistics from `src/solver/breakthrough_out/`** until this is resolved in git; owner: author. `src/docs/review/_source/report_stats_tables.py` rejects any results file that is not a 24-row single-run table.
- `P7_2026-07-03-conc5-flow0.10.png` was a truncated write (852,244 B) in both trees; regenerated 2026-08-09 by `src/docs/review/_source/regen_truncated_p7.py`, which refits that one run and only accepts the new PNG if the refit reproduces the committed optimum (it did, to 8.6e-7). No results CSV was touched.
...
```bash
# The venv at the repo root is `.venv/`, not `venv/` (numpy, scipy, matplotlib,
# pandas, python-docx, openpyxl, pillow). Nothing on PATH has scipy.
source .venv/Scripts/activate    # Git Bash on Windows
pip install -r requirements.txt  # numpy, scipy, matplotlib, pandas + report tooling
...
The five bench runs in `src/solver/data/new runs/` (run 3/4/5/6/8) plus sixteen bench runs in `src/solver/data/newest runs/` (2026-06-26 – 2026-07-31) are the only measured data — 21 real runs total, see `src/docs/experimental-results.md` §1–9 and §10 respectively. **`experimental-results.md` §10 still documents only the first nine of the sixteen** — the seven replicates logged 2026-07-17 onward are covered by `src/docs/experiments_passport.md` and by §5 of the Final Report, not by §10. Refresh results with:

```bash
python src/solver/new_runs_pipeline.py            # 16 usable newest runs -> breakthrough_out/<run_id>/ (currently targets `newest runs/` only — see note below)
...
python src/solver/build_experiments_passport.py   # provenance ledger + §5 tables -> src/docs/experiments_passport.{json,md} (reads committed artefacts; refits nothing)
```
...
### Editing the Final Report .docx

`src/docs/review/_source/` holds the report tooling. **Word must be closed** — it takes an exclusive write lock (`src/~$2_PI05_Final_Report.docx`). Every script works on a copy and only overwrites the report once each step succeeds; `--dry-run` stops before the copy-back.

```bash
python src/docs/review/_source/insert_figures_and_stats.py --dry-run   # figure/statistics pass
python src/docs/review/_source/verify_figures_and_stats.py <docx>      # 35 checks, incl. 12,148 table cells
python src/docs/review/_source/extract_report.py                       # refresh report-extracted.md mirror
```

`docxlib.py` is the shared lxml/python-docx helper layer; `insert_experiments_section.py` (§5, applied 2026-08-09) and `insert_figures_and_stats.py` (figures + statistics, applied 2026-08-09) are **one-shot, non-idempotent migrations** — write a new module rather than re-running or extending them. Table data comes from `report_stats_tables.py`, which reads committed CSVs and the ANOVA workbook and refits nothing. Figures are downscaled into a gitignored `_figcache/`; the committed PNGs are never modified.
