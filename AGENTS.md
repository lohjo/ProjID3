...
This file provides guidance to Codex (Codex.ai/code) when working with code in this repository. It mirrors `CLAUDE.md` — keep the two in sync; when you change a fact in one, change it in the other.
...
### 6. Twin file
- `CLAUDE.md` (the Claude Code twin) mirrors this file. When you change a fact here, **mirror the change into `CLAUDE.md`** so they don't drift.
...
- `src/docs/sensitivity-analysis.md` — cluster-ANOVA sensitivity analysis (Kleijnen & Helton 1999a CMNs/CLs; Saltelli et al. 2000 SRC). Live source-of-truth for the flow/concentration effect on breakthrough time and capacity, and for empirical-parameter sensitivity. Basis: **n = 16 grid / n = 21 pooled** (rerun 2026-08-05 on the expanded `newest runs/`). Generated by `src/solver/sensitivity_anova.py`; tables in `src/docs/sensitivity_anova_tables.xlsx`; figures in `src/img/generated/sensitivity/`; numeric claims re-verified by `src/docs/verify_sensitivity_doc.py`. Standing flags: (i) **q_dyn is not currently measurable** — 78.4 % of its variance is pure replicate error, replicates differ up to 3.04×, so the earlier "concentration sets capacity" claim is **withdrawn**; fix the acquisition endpoint (run to fixed C/C₀ = 0.98) before any capacity conclusion; (ii) M10/M11 `k₀` and `h` are **not independently identifiable** (ρ_S = 0.984 / 0.971) — report `κ = k₀τ₀^(−h)`, not `k₀`; (iii) **do not pool** the 5 old runs with the grid for inference (bed L and i.d. differ systematically and every η² falls); (iv) `L_MTZ` has a hard floor of 0.5·L_bed and only 4.0 % CV, so its effects are never physically large; (v) `2026-07-22-conc10-flow0.10.csv` **is mis-named** — its header says 150 ml/min and its t_b confirms it; treat as 0.15 lpm and rename at source.
...
- `src/docs/archive (DO NOT OPEN)/` — superseded old-scope. Do **not** read or edit. This now contains the old `derivation.md`, `experimental-results.md`, `CO2-adsorption-model.md`, `equation_compendium.md`, `literature-survey-summary.md`, `my-research-paper.md`, `study_plan.md`, `research.md` (in `maybe_archive/`). Earlier CLAUDE.md/AGENTS.md versions wrongly listed these six as live under `src/docs/` — they are **not**.
